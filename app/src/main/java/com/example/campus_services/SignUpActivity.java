...
public class SignUpActivity extends AppCompatActivity {
    ...
    private void Register(){
        ...
        while(i<email.length() && email.charAt(i)!='@')
            i++;
        if(i==email.length()){
            Toast.makeText(SignUpActivity.this,"Enter valid CHARUSAT email address!",Toast.LENGTH_SHORT).show();
            return;
        }
        if(!email.substring(i).equals(t.substring(9))){
            Toast.makeText(SignUpActivity.this,"Only CHARUSAT email address is valid!",Toast.LENGTH_SHORT).show();
            return;
        }
